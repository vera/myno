...
#define MOISTURE_MAX 4095
#define MOISTURE_MIN 2000
#define RAIN_MIN 2000
#define RAIN_MAX 4095
...
void out_rain_1(){
  ...
  Serial.print("Rain ");
  Serial.println(rain);
  float rain_result =(1 - (rain - int(RAIN_MIN))/(float(RAIN_MAX)-float(RAIN_MIN)))*100;
  sprintf(rain_msg, "%.2f", rain_result);
  client.publish(RAIN1_TOPIC_P, rain_msg);
}

void out_moisture_1(){
  char moisture_msg[10];
  int moisture = analogRead(moisture_out);
  Serial.print("Moisture ");
  Serial.println(moisture);
  float moisture_result = (1 - (moisture - int(MOISTURE_MIN))/(float(MOISTURE_MAX)-float(MOISTURE_MIN)))*100;
  sprintf(moisture_msg, "%.2f", moisture_result);
